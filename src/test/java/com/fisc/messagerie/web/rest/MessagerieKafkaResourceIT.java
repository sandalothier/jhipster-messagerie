package com.fisc.messagerie.web.rest;

import com.fisc.messagerie.config.KafkaProperties;
...
class MessagerieKafkaResourceIT {
    ...
    void setup() {
        ...
        MessagerieKafkaResource kafkaResource = new MessagerieKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/messagerie-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/messagerie-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
